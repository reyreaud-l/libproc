...
std::vector<fs::path> get_proc_dirs(const fs::path& root) {
  if (!fs::is_directory(root)) {
    throw std::invalid_argument("Expected directory as 1st argument, got: " + root.string());
  }
...
fs::path get_proc_dir(const fs::path& root, int uid) {
  ...
  throw std::invalid_argument("Could not find a process for uid: " + std::to_string(uid));
}
}  // namespace plib
