...
TEST(Fs, get_proc_dir_no_dir) {
  ASSERT_THROW(plib::get_proc_dir(fs::path("/nowhere"), 0), std::invalid_argument);
}

TEST(Fs, get_proc_dir_no_uid) {
  // This UID really is random. But hey.
  ASSERT_THROW(plib::get_proc_dir(plib::get_proc_root(), 9999999), std::invalid_argument);
}

TEST(Fs, get_proc_dirs_no_dir) {
  ASSERT_THROW(plib::get_proc_dirs(fs::path("/nowhere")), std::invalid_argument);
}
...
TEST(Fs, resolve_father) {
  ...
  auto init = std::find_if(procs.begin(), procs.end(),
